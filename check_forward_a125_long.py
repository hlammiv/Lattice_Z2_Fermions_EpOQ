"""4-instance forward-time PT-exact at a_τ=0.125 for proper SEM.
After the refactor that removed central-diff, this is the canonical
methodology-paper test. Sign problem is severe with forward (⟨sgn⟩≈0.02 from
the single-instance pilot), so the SEMs will be large — we need 4 instances
to detect bias separately from noise."""
...
N_PER_SECTOR = 50
A_TAU = 0.125
CKPT_DIR = "/tmp/check_forward_a125_long_ckpt"
...
def main():
    ...
    print("=" * 80, flush=True)
    print(f"FORWARD-TIME long check @ a_τ={A_TAU}, m={M_HAM}, β={BETA}")
    print(f"  {N_INSTANCES} indep PT-forward instances, "
          f"warm={N_WARMUP} swp={N_SWEEPS}, n_per_sector={N_PER_SECTOR}")
    print("=" * 80, flush=True)
    ...
    print()
    print("=" * 70, flush=True)
    print(f"SUMMARY  @ a_τ={A_TAU}, FORWARD-time, {N_INSTANCES} indep PT")
    print("=" * 70)
    print(f"{'t':>4} | {'C(t)':>22} | {'ED ref':>10} | {'sigmas':>7}")
    for t in times:
