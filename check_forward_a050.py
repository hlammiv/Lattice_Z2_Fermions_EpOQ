"""Verify the forward-time pipeline at a_τ=0.5 (mild K_E=0.39, m_action=1.0).
Goal: confirm the methodology produces ED when the sign problem is small.
If C(t) matches ED at a_τ=0.5, the code is correct and the a_τ=0.125 issue is
purely sign-problem variance. Single PT instance is enough — fast."""
...
def run_one_PT(seed, cached_UOU, n_E_plaq, times):
    ...
    return {'C': Cs, 'n_diag': n_diag, 'sum_denom': sum_denom,
            ...
            'K_E_target': K_E_target, 'm_action': m_action}


def main():
    times = [0.0, 0.5, 1.0, 2.0]
    print("=" * 80, flush=True)
    print(f"FORWARD-time pipeline check @ a_τ={A_TAU}, m_Ham={M_HAM}, β={BETA}")
    print(f"  K_E_target={-0.5*np.log(np.tanh(A_TAU*G_E_HAM)):.4f}, "
          f"K_M={A_TAU*G_M_HAM:.4f}, m_action={A_TAU*M_HAM:.4f}")
